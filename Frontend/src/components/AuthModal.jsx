  ...
  const handleConfirmarRolGoogle = async (rolElegido) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          data.error ||
            "Ocurrió un problema al crear tu cuenta con Google. Inténtalo de nuevo.",
        );
        return;
      }
  ...
  const handleRegistro = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          data.error ||
            "Tuvimos un problema al crear tu cuenta. Intenta de nuevo más tarde.",
        );
        return;
      }
      login(data.token, data.usuario);
            ...
            >
              ...
              <div style={{ textAlign: "center" }}>
                <button
                  ...
                  style={{ fontSize: 13 }}
                >
                  <AppIcon name="arrowLeft" size={15} /> Volver al inicio de
                  sesión
                </button>
              </div>
            </form>
          ...
          {!loginExito && !pendienteGoogle && vista !== "recuperar" && (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => {
                  setVista(vista === "login" ? "registro" : "login");
                  setError("");
                }}
                style={{
                  fontSize: 14,
