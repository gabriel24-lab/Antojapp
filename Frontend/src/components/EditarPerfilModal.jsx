...
export default function EditarPerfilModal({ onCerrar }) {
  ...
  const handleCambiarPassword = async (e) => {
    ...
    if (!passwordActual)              return setErrorPassword("Ingresa tu contraseña actual");
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,.5)", marginTop: 3 }}>
              {vista === "datos" ? "Actualiza tu información" : "Cambia tu contraseña"}
            </div>
          </div>
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          {/* ── PESTAÑA DATOS ── */}
          {vista === "datos" && (
            <form onSubmit={handleGuardarDatos} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 6 }}>
                <div
                  ...
                  style={{
                    ...
                    background: imagenMostrar ? `url(${imagenMostrar}) center/cover` : "var(--brand)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 32, fontWeight: 800, color: "var(--surface)",
                ...
                >
                  ...
                  <div style={{
                    ...
                    background: "var(--text-1)", border: "2px solid var(--surface)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: "var(--surface)",
                  }}>
              ...
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: "var(--text-2)", display: "block", marginBottom: 5 }}>Nombre</label>
                <input className="input" type="text" placeholder="Tu nombre" value={nombre} onChange={e => { setNombre(e.target.value); setErrorDatos(""); }} required maxLength={100} />
              </div>
              ...
              {errorDatos && <div style={{ fontSize: 13, color: "var(--red)", background: "var(--red-bg)", padding: "9px 12px", borderRadius: 8 }}>{errorDatos}</div>}

              <button className="btn-primary" type="submit" disabled={cargandoDatos} style={{ width: "100%", marginTop: 2 }}>
            ...
            <>
              {exitoPassword ? (
                <div style={{ background: "var(--green-bg)", border: "1px solid var(--green)", borderRadius: 10, padding: "16px", textAlign: "center" }}>
                  <div style={{ marginBottom: 8, color: "var(--green)" }}><AppIcon name="check" size={30} /></div>
                  <div style={{ fontSize: 14, color: "var(--green)", fontWeight: 500 }}>{exitoPassword}</div>
                </div>
              ) : (
                <form onSubmit={handleCambiarPassword} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 500, color: "var(--text-2)", display: "block", marginBottom: 5 }}>Contraseña actual</label>
                    <input className="input" type="password" placeholder="••••••••" value={passwordActual} onChange={e => { setPasswordActual(e.target.value); setErrorPassword(""); }} required />
                  </div>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 500, color: "var(--text-2)", display: "block", marginBottom: 5 }}>Nueva contraseña</label>
                    <input className="input" type="password" placeholder="Mín. 8 caracteres, 1 mayúscula y 1 número" value={passwordNueva} onChange={e => { setPasswordNueva(e.target.value); setErrorPassword(""); }} required />
                  </div>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 500, color: "var(--text-2)", display: "block", marginBottom: 5 }}>Confirmar nueva contraseña</label>
                    <input className="input" type="password" placeholder="Repite la contraseña" value={passwordConfirmar} onChange={e => { setPasswordConfirmar(e.target.value); setErrorPassword(""); }} required />
                  </div>
                  {errorPassword && <div style={{ fontSize: 13, color: "var(--red)", background: "var(--red-bg)", padding: "9px 12px", borderRadius: 8 }}>{errorPassword}</div>}
                  <button className="btn-primary" type="submit" disabled={cargandoPassword} style={{ width: "100%", marginTop: 2 }}>
