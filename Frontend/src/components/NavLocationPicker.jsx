...
export default function NavLocationPicker({
  ...
  return (
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      ...
      >
        <AppIcon name="mapPin" size={14} color="#E8460A" />
        <div style={{ overflow: "hidden", textAlign: "left", flex: 1, minWidth: 0 }}>
          {labelPrincipal ? (
            <>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.2 }}>
                {labelPrincipal}
              </div>
            </>
          ) : (
          ...
          <div style={{
            ...
            <div style={{ padding: "14px 16px 10px", borderBottom: "1px solid #F0EBE5", background: "#FDFAF8" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {paso !== "pais" && (
                  <button
                    ...
                    style={{ background: "none", border: "none", cursor: "pointer", padding: "2px 4px", color: "#A8988A", display: "flex", alignItems: "center" }}
                  >
                    <AppIcon name="chevronLeft" size={14} />
                  </button>
                )}
                <span style={{ fontSize: 13, fontWeight: 700, color: "#1A1208", flex: 1 }}>
                  {titulos[paso]}
                </span>
                {/* Breadcrumb bandera */}
                {paisTemp && paso !== "pais" && (
              ...
              {(paisTemp || deptTemp) && (
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                  {paisTemp && (
                    <span style={{ fontSize: 10, background: "#FFF0EB", color: "#E8460A", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>
                      {paisTemp.nombre}
                    </span>
                  )}
                  {deptTemp && (
            ...
            <div style={{ padding: "8px 12px", borderBottom: "1px solid #F0EBE5" }}>
              <div style={{ position: "relative" }}>
                <AppIcon name="search" size={13} style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", color: "#A8988A" }} />
                <input
                  autoFocus
                  type="text"
                  placeholder={
            ...
            {paisSeleccionado && (
              <div style={{ padding: "10px 14px", borderTop: "1px solid #F0EBE5", background: "#FDFAF8", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ fontSize: 11, color: "#A8988A", display: "flex", alignItems: "center", gap: 5 }}>
                  <AppIcon name="mapPin" size={11} color="#E8460A" />
                  <span>
                ...
                <button
                  onClick={limpiarTodo}
                  style={{ fontSize: 11, color: "#E8460A", background: "none", border: "none", cursor: "pointer", fontWeight: 600, padding: "2px 6px" }}
                >
                  Limpiar
                </button>
              </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        background: activo ? "#FFF0EB" : hover ? "#F7F4F1" : "transparent",
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {sub && <div style={{ fontSize: 11, color: "#A8988A", marginTop: 1 }}>{sub}</div>}
      </div>
      {activo && !chevron && <AppIcon name="check" size={14} color="#E8460A" style={{ flexShrink: 0 }} />}
...
function EmptyMsg({ texto }) {
  return (
    <div style={{ padding: "20px", textAlign: "center", color: "#A8988A", fontSize: 13 }}>
      {texto}
    </div>
  );
}
